      ...
      if (logs) {
        logs.forEach((log) => {
          ...
          if (log.parsed_output?.includes?.("The presentation is complete")) {
            ["planning", "preferences", "content", "design", "validation"].forEach((p) => completed.add(p));
          }
        });
        state.completedPhases = Array.from(completed);
        state.currentPhase = completed.has("validation") ? "completed" : Array.from(completed).pop() || "planning";
      }

      if (logs) {
        const planningLog = logs.find((log) => log.agent_name === "planning_agent" && log.parsed_output);
        if (planningLog) {
          try {
            const parsed = typeof planningLog.parsed_output === "string" ? JSON.parse(planningLog.parsed_output) : planningLog.parsed_output;
...
export const { setPresentationState, resetPresentationState } = presentationSlice.actions;
