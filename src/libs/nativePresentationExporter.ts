...
const createRenderIframe = (htmlBody: string): Promise<HTMLIFrameElement> => {
  return new Promise((resolve, reject) => {
    ...
    iframe.onload = () => {
      ...
      setTimeout(() => {
        doc.fonts.ready.then(() => setTimeout(() => resolve(iframe), 1000))
          .catch(err => {
    ...
    iframe.onerror = (err) => reject(new Error(`Iframe failed to load: ${err}`));
 ...
 */
function processTextNode(node: Node, defaultStyle: CSSStyleDeclaration): PptxGenJS.TextProps[] {
  ...
  if (node.nodeType === Node.TEXT_NODE && node.textContent?.trim()) {
    textRuns.push({
      ...
      options: {
        fontFace: defaultStyle.fontFamily.split(',')[0].trim().replace(/"/g, ''),
...
async function renderNodeToPptx(node: Element, slide: PptxGenJS.Slide, slideRect: DOMRect) {
  if (!(node instanceof HTMLElement)) return;

  const style = window.getComputedStyle(node);
  const rect = node.getBoundingClientRect();

  if (style.display === 'none' || style.visibility === 'hidden' || node.tagName === 'SCRIPT' || node.tagName === 'STYLE' || rect.width <= 1 || rect.height <= 1 || node.classList.contains('glass-orb')) {
    return;
  }
  ...
  // Handle DIVs as Shapes
    ...
    if (bgColor) {
      slide.addShape(PptxGenJS.shapes.RECTANGLE, {
        x: pxToInches(relRect.x), y: pxToInches(relRect.y), w: pxToInches(relRect.w), h: pxToInches(relRect.h),
        fill: { color: bgColor }
      });
    }
  }
  ...
  // Handle Icons (Rasterize as fallback)
    ...
    slide.addImage({ data: imageData, x: pxToInches(relRect.x), y: pxToInches(relRect.y), w: pxToInches(relRect.w), h: pxToInches(relRect.h) });
    stopRecursion = true;
  }
  ...
  // Handle Canvas/Charts (Corrected Logic)
    ...
    if (chartConfig) {
      console.log("Native Chart Export: Found chart config, attempting to render.", chartConfig);
      try {
        const chartData = chartConfig.data.datasets.map((ds: any) => ({ name: ds.label, labels: chartConfig.data.labels, values: ds.data }));
        ...
        console.error("Native Chart Export: Failed to parse or render native chart, falling back to image.", e);
      }
    }

    if (!nativeChartRendered) {
      console.warn("Native Chart Export: No config found or native render failed. Falling back to image capture.");
      const imageData = await captureElementAsImage(node);
      slide.addImage({ data: imageData, x: pxToInches(relRect.x), y: pxToInches(relRect.y), w: pxToInches(relRect.w), h: pxToInches(relRect.h) });
    }
    stopRecursion = true;
 ...
 */
export const handleNativePptxExport = async (slidesData: SlideData[], options: ExportOptions = {}): Promise<{ success: boolean; error?: string }> => {
  ...
  try {
    for (const slideData of slidesData) {
      ...
      try {
        ...
        if (!slideContainer || !slideRoot) throw new Error(`Slide ${slideData.slide_index}: .slide-container or .slide not found.`);
        ...
        if (slideRootStyle.backgroundImage.includes('gradient')) {
          console.log(`Slide ${slideData.slide_index}: Complex background detected, capturing as image.`);
          const bgImageData = await captureElementAsImage(slideRoot);
          pptxSlide.addImage({ data: bgImageData, x: 0, y: 0, w: '100%', h: '100%' });
          
          const contentWrapper = slideRoot.querySelector('.quote-wrapper') || slideRoot.querySelector('.content-wrapper');
          if (contentWrapper) {
            await renderNodeToPptx(contentWrapper, pptxSlide, slideRect);
          }
        } else {
          pptxSlide.background = { color: parseColor(slideRootStyle.backgroundColor) };
    ...
    await pptx.writeFile({ fileName: options.fileName || 'presentation-native.pptx' });
    return { success: true };
