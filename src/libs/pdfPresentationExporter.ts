 ...
 */
const captureElementAsImage = async (element: HTMLElement, quality: number = 0.92): Promise<string> => {
...
const getPDFDimensions = (format: string, orientation: string) => {
  ...
  const dimensions = formats[format as keyof typeof formats] || formats.presentation;
...
export const handlePDFExport = async (
  ...
  try {
    // Get PDF dimensions
    const { width: pageWidth, height: pageHeight } = getPDFDimensions(format, orientation);
    
    // Create PDF document
    const pdf = new jsPDF({
      orientation: orientation as 'landscape' | 'portrait',
      unit: 'mm',
      format: format === 'presentation' ? [pageWidth, pageHeight] : format as any
    });
    ...
    for (let i = 0; i < slidesData.length; i++) {
      ...
      try {
        ...
        const container = iframe.contentWindow?.document.querySelector('.slide-container') as HTMLElement;
        
        if (!container) {
          throw new Error(`Slide ${slide.slide_index}: .slide-container element not found.`);
        }
        ...
        pdf.text(`Slide ${i + 1} of ${slidesData.length}`, pageWidth - margin, margin - 2, { align: 'right' });
    ...
    const errorMessage = error instanceof Error ? error.message : 'An unknown error occurred during PDF export.';
